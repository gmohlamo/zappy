...
/*   Updated: 2020/01/17 20:33:38 by gmohlamo         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
enum e_operations			level_three_check(t_game *game, t_client *client)
{
	...
	if (count_resources(game, client, linemate) < 2)
		return (incantation_failure(client));
	else if (count_resources(game, client, sibur) < 1)
		return (incantation_failure(client));
	if (count_resources(game, client, phiras) < 1)
		return (incantation_failure(client));
	return (incantation);
}